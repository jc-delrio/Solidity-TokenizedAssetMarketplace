...
describe("Bateria Test Acquisition", () => {
    ...
    it("TEST 3 - Solo Fondo de Inversión puede desabastecer activos", async () => {
        ...
        const hashPDelist = await acquisition.write.delistAsset([1n, 1n], { account: fund.account });
        const receiptPDelist = await publicClient.getTransactionReceipt({ hash: hashPDelist });

        const eventsP = await publicClient.getContractEvents({
            address: acquisition.address,
            abi: acquisition.abi,
            eventName: 'AssetDelisted',
            fromBlock: receiptPDelist.blockNumber,
        });
        assert.equal(eventsP.length, 1);
        const argsP = eventsP[0].args;
        assert.equal(argsP.id, 1n);
        assert.equal(argsP.desupply, 1n);
        assert.equal(argsP.value, 0n);
    });
